...
def run_single_mcd(test_data, test_labels):
    ...
    # Anomalie-Rate im aktuellen Split bestimmen (Anomalie = 1)
    n_anomalies = np.sum(y_test == 1)
    contamination = max(0.01, n_anomalies / len(X_test))

    # Modell fitten - Unsupervised auf dem Test-Set
    clf = MCD(contamination=contamination, random_state=42)
    clf.fit(X_test)
...
def start_mcd_benchmark(dataset_name, num_splits=500):
    ...
    # Pfad-Logik basierend auf deinem Repository-Struktur
    ...
    print(f"--- Starte MCD Benchmark für: {dataset_name} ---")
    print(f"Anzahl der Durchläufe: {num_splits}")
    print(f"Suche Datei unter: {mat_file_path}")
    ...
    for i in range(num_splits):
        try:
            # Erzeuge den 50/50 Split (identisch zu InterCont)
            # train wird hier ignoriert, da MCD unsupervised auf test_data arbeitet
            _, test_data, test_labels = dl.build_train_test_generic_matfile(mat_file_path)

            # Führe MCD auf diesem spezifischen Split aus
            f1, f1_m, auc, prc = run_single_mcd(test_data, test_labels)

            all_f1.append(f1)
            all_f1_m.append(f1_m)
            all_auc.append(auc)
            all_prc.append(prc)
    ...
    if len(all_f1) > 0:
        ...
        print("Keine erfolgreichen Durchläufe.")

...
The PC has 65536 MB RAM and 24326 MB of VRAM. 
...
For this semisupervised approach does k-NN learn a mapping of the training data.
In the next step, the method calculated the distances between the test and the k nearest training data point.
The resulting distances are then used to calculate the anomaly score for each test data point.
Therefor it determines the largest distance to the k nearest neighbors and uses this distance as the anomaly score.
The contamination parameter is set to 15.4 because all outliers are included in the test dataset.
...
To make the results comparable, I mikmiked a training and testing phase.
The training phase, MCD estimales the covariance matrix and the mean of the normal data.
I use the same Resample method as the authors of InterCont. 
The covariance matrix and the mean is used for the testing phase which is the residual dataset which include all outliers.
The datapoints with the highest Mahalanobis distance to the mean are classified as outliers, the treshold is set to 15.4\% because the testdataset contains all outliers and normal data.
The cycle is repeated 500 times, because the InterCont method is also run 500 times with different shuffles of the data.
The resulting metrics are the averaged over the 500 runs and the standard deviation is calculated as well.

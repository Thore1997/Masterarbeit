...
To exert the framwork discussed above, I will begin with the data augmentation techniques the authors chose.
...
The single coumns of the table is taken and split up accourdingly to its hyperparameters.
Resulting in two vectors of different sizes that which complement each other.
The anchor is represented by the larger vector bi and the complementary vector ai represents the positive sample.
The negative samples are the positive samples of other columns, hence the name internal contrastive learning.
...
The authers defined the number of nodes as dependent on the embedding size u.
...
Later in the thesis I will explain how and why the normalization took place.
The numerator resembles the pull.
It is product of the positive pair (anchor and positive sample) and defiding it by tau.

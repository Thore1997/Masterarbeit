...
The denominator is dominated by the true negatives. 
...
As mentioned above, precision and recall share a trade-off relation as well.
If one metric increases, the other will decrease.
In other words, there is a trade-off between every outlier (recall) and detecting it correctly (precision)
...
Suppose a naive model guessing the number of outlier in a given dataset.
Statistically the results depend on the prevelance of the share of outliers and inliers.
...
\todo{insert baseline argument}
, designed for imbalanced datasets, emphasizes the minority class by plotting precision against recall across various thresholds. 

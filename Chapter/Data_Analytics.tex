...
Binary variables are hard to implement into the methods and in the dataset supplied in the ODDS Benchmarkt repository they exluded all binary variables, so I removed them too.
...
Wich methods can be used is determined by the type of missing data.
...
Data is missing not at random (MNAR) if it missing because of its values.
All types of missing data can be met with all the methods described except of MNAR.
By using the imputation methods I could be introducing a bias.
But the goal of the imputation is to maintain as much as possible data, not to analyse relationsships. 
...
Therefor I chose the median imputation, because the median is a outlier robust parameter.

...
The k-nearest neighbor (kNN) algorithm is a nonparametic classification algorithm. It is been known for its simplicity and computational and low computation time.
It is a supervised learning algorithm, that means, that a labled traning dataset is been used to train the model and afterwards a class of unlabled data can be predicted.
It is a "lazy learning" algorithm. It does not achieve generalizations. So the entire fundamental traningset needs be used for traning, because it does not calculate generalization.
Outlier detection is a classical classification problem. 

There are tweo steps in classification: 
\begin{enumerate}
\item Learning Step: A clasifier is constructed by using the datatset
\item Assessment of the classifier
\end{enumerate}

When a new unlabled data is presented into the dataset, the k-NN algorithm determines in wich class the unlabled data should go 
by determining the wich classes the neighboring belong to.
...
KNN is used as a classifier. It is used to classify data based on closest or neighboring training examples in a given region. 
For continuous data, it uses the Euclidean distance to calculate its nearest neighbors.
...
The biggest disadvantage of k-NN is the determinantion of $K$.




One of the major steps in KNN is determining the parameter K.
 KNN is a lazy learning algorithm as it doesn’t learn from the training data, it implies memorizes it and then uses that data to classify the new input.

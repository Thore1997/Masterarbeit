...
First I will describe the goal and mechanics of MCD and highlight the downside of it.
To solve the problems of MCD I will introduce the FAST-MCD algorithm, which I will implement in my experiments.
After describing the algorithms steps I will proceed with the advantages and disadvantages and explain why I chose this method as a benchmark in my experiments.
...
\citet{rousseeuw1999fast} introduced the FAST-MCD algorithm, which utilizes concentration steps (C-Steps) to iteratively improve the subset selection and pruning the process to mitigate the estimation of estimating $\binom{n}{h}$ combinations.
The general MCD algorithms is constructed as follows:
\begin{enumerate}
...
The C-Step operates by calculating the Mahalanobis distances for all $n$ observations based on the chosen subset's parameters, then forming a new subset from the $h$ observations with the smallest distances. 
...
The C-Step is constructed as follows:
\begin{enumerate}
    \item   Initialize by drawing a random number $p+1$ of subsamples $J$ and calculate the sample mean and the sample covariance matrix. 
            If the resulting $det(S0)=0$ then add another random observation until $det(S0)>0$. 
...
The two concentration steps will be applied to all initial subsets, followed by a full convergence of the ten most promising candidates. 
...
\subsubsection*{Variants and Evaluation}
